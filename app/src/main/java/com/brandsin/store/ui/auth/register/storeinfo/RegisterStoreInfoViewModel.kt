...
import com.brandsin.store.model.auth.*
...
class RegisterStoreInfoViewModel : BaseViewModel()
{
    ...
    var storeTagsRequest = StoreTagsRequest()
    var tagsResponse = StoreTagsResponse()
    var tagsList : ArrayList<StoreTagsItem> = ArrayList()

    fun getStoreTypes() {
        requestCall<StoreTypeResponse?>({ withContext(Dispatchers.IO) { return@withContext getApiRepo()
    ...
    fun getTags() {
        ...
        responseCall.enqueue(object : Callback<StoreTagsResponse?> {
            override fun onResponse(call: Call<StoreTagsResponse?>, response: Response<StoreTagsResponse?>) {
...
//                setValue(Codes.EMPTY_STORE_STORE_THUM)
//            }
    ...
    fun onTagsClicked() {
        if (storeRequest.categories == null || storeRequest.categories!!.isEmpty()) {
            setValue(Codes.EMPTY_TYPE)
        }else {
