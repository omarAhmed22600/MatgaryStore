...
class DialogStoreTagsFragment : DialogFragment(), Observer<Any?> {
    ...
    private lateinit var binding: DialogStoreTagBinding
            ...
            arguments != null -> {
                when {
                    requireArguments().containsKey(Params.STORE_TAGS) -> {
                        response =
                            (requireArguments().getSerializable(Params.STORE_TAGS) as StoreTagsResponse?)!!
                        if (requireArguments().getSerializable(Params.STORE_TAGS_DATA) != null) {
                            storeRequest =
                                requireArguments().getSerializable(Params.STORE_TAGS_DATA) as StoreRegister
                        }
                        if (requireArguments().getSerializable(Params.STORE_TAGS_DATA_EDIT) != null) {
                            updateRequest =
                                requireArguments().getSerializable(Params.STORE_TAGS_DATA_EDIT) as UpdateStoreRequest
                        }
                    }
                }
            }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this)[StoreTagsViewModel::class.java]
        ...
        viewModel.tagsAdapter.orderAddonsLiveData.observe(this) { o ->
        ...
        when (value) {
            null -> return
                ...
                Codes.CONFIRM_CLICKED -> {
                    ...
                    intent.putExtra("storeTagNames", storeTagNames.joinToString { it })
