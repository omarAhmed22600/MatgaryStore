            ...
            intent.hasExtra(Params.STORE_REGISTER) -> {
                when {
                    intent.getSerializableExtra(Params.STORE_REGISTER) != null -> {
                        val storeData: StoreRegister = intent.extras!!.getSerializable(Params.STORE_REGISTER) as StoreRegister
                ...
                when (it) {
                                ...
                                when {
                                    intent.getSerializableExtra(Params.STORE_REGISTER) != null -> {
                                        ...
                                        for (item in viewModel.tagsList) {
                                            if (viewModel.storeRequest.tags!!.contains(item.id!!.toInt())) {
                                                storeTagsNames.add(item.name.toString())
                                            }
                                        }
                                        binding.etStoreTags.text = storeTagsNames.joinToString { it -> "$it" }
                                    }
                                }
                            }
                        }
                    ...
                    Codes.STORE_CATEGORIES_CLICKED -> {
                        ...
                        if (viewModel.storeRequest.categories != null) {
                            bundle.putSerializable(Params.STORE_CATEGORIES_DATA, viewModel.storeRequest)
                        }
                        Utils.startDialogActivity(this, DialogStoreTypesFragment::class.java.name, Codes.DIALOG_STORE_CATEGORY_CODE, bundle)
                    }
                    Codes.STORE_TAGS_CLICKED -> {
                        ...
                        Utils.startDialogActivity(this, DialogStoreTagsFragment::class.java.name, Codes.DIALOG_STORE_TAGS_CODE, bundle)
                    }
                ...
                if (data.hasExtra(Params.DIALOG_CLICK_ACTION)) {
                    when {
                        data.getIntExtra(Params.DIALOG_CLICK_ACTION, 0) == 1 -> {
                            ...
                            viewModel.storeRequest.categories =  data.getIntegerArrayListExtra("storeCategoryId")
                            viewModel.getTags()
                        }
                    }
                }
                ...
                if (data.hasExtra(Params.DIALOG_CLICK_ACTION)) {
                    when {
                        data.getIntExtra(Params.DIALOG_CLICK_ACTION, 0) == 1 -> {
                            ...
                            viewModel.storeRequest.tags =  data.getIntegerArrayListExtra("storeTagId")
                        }
                    }
                }
            }
    ...
    fun pickImage(requestCode: Int) {
        ...
        when {
            PermissionUtil.hasImagePermission(this) -> {
                ...
                observe(PermissionUtil.requestPermission(this, PermissionUtil.getImagePermissions())) {
                    when (it) {
